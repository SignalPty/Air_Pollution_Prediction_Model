# -*- coding: utf-8 -*-
"""
Streamlit Air Pollution Prediction App (Fixed for Hosting)
"""
...
# ---- Load model safely ----
@st.cache_resource
def load_model(path="Air_Pollution_Model.pkl"):
    ...
    try:
        ...
        st.error("⚠️ Missing library needed to load the model.")
...
if st.button("Predict Air Quality Index"):
    if model is None:
        ...
        try:
            ...
            # Color-coded AQI
            if aqi_pred <= 50: st.success("Air Quality: Good 🟩")
            elif aqi_pred <= 100: st.warning("Air Quality: Moderate 🟨")
            elif aqi_pred <= 150: st.error("Unhealthy for Sensitive Groups 🟧")
            elif aqi_pred <= 200: st.error("Unhealthy 🟥")
            elif aqi_pred <= 300: st.error("Very Unhealthy 🟪")
